...
with st.sidebar:
    ...
    available_to_add = [u['name'] for u in all_users if u['name'] not in friend_names and u['name'] != current_user_name]
            ...
            for item in friend_reviews:
                ...
                with st.container(border=True):
                    ...
                    with col2:
                        ...
                        st.caption(f"{date_obj.strftime('%d %b %Y à %H:%M')} · {content['type']} sur {content['platform']}")

                    st.markdown(f"##### Note : {'⭐' * review['rating']}{'☆' * (10 - review['rating'])} ({review['rating']}/10)")
...
with right_sidebar:
    ...
    MOCK_GLOBAL_API_DATA = [
        ...
        {"source": "Rotten Tomatoes", "title": "Fallout (Série)", "snippet": "Score de 94% - Une adaptation fidèle et inspirée."},
        {"source": "JeuxVideo.com", "title": "Dragon's Dogma 2", "snippet": "Une aventure unique mais techniquement datée. 16/20."},
